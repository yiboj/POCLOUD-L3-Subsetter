...
def run(args=None):
    ...
    if len(downloads_data) < 1:
      print("\n\n*** Could not find files to subset, please try again ! ***\n\n")
      exit()
    ...
    indexlat = np.where((latValue >= float(abox[1])) & (latValue <= float(abox[3])))
    indexlon = np.where((lonValue >= float(abox[0])) & (lonValue <= float(abox[2])))
    strOpendap = ""

    for iCord in ds.coords:
      if 'lat' in iCord.lower() and '_' not in iCord.lower():
        strOpendap = strOpendap + '/'+iCord+'%5B'+str(indexlat[0][0])+':1:'+str(indexlat[0][-1])+'%5D;'
      elif 'lon' in iCord.lower() and '_' not in iCord.lower():
        strOpendap = strOpendap + '/'+iCord+'%5B'+str(indexlon[0][0])+':1:'+str(indexlon[0][-1])+'%5D;'
      elif 'time' in iCord.lower() and '_' not in iCord.lower():
        strOpendap = strOpendap + '/'+iCord+'%5B0'+':1:'+str(len(ds[iCord].values)-1)+'%5D;'

    for iCord in ds.coords:
      if '_' in iCord.lower():
        strOpendap = strOpendap + '/'+iCord
        for id in ds[iCord].dims:
          if 'lat' in id:
            strOpendap = strOpendap + '%5B'+str(indexlat[0][0])+':1:'+str(indexlat[0][-1])+'%5D'
          elif 'lon' in id:
            strOpendap = strOpendap + '%5B'+str(indexlon[0][0])+':1:'+str(indexlon[0][-1])+'%5D'
          else:
            strOpendap = strOpendap + '%5B0'+':1:'+str(len(ds[id].values)-1)+'%5D'
        strOpendap = strOpendap + ';'

    for ivar in ds.data_vars:
      strOpendap = strOpendap + '/'+ivar
      for iCord in ds[ivar].dims:
        if 'lat' in iCord:
          strOpendap = strOpendap + '%5B'+str(indexlat[0][0])+':1:'+str(indexlat[0][-1])+'%5D'
        elif 'lon' in iCord:
          strOpendap = strOpendap + '%5B'+str(indexlon[0][0])+':1:'+str(indexlon[0][-1])+'%5D'
        else:
          strOpendap = strOpendap + '%5B0'+':1:'+str(len(ds[iCord].values)-1)+'%5D'
      strOpendap = strOpendap + ';'

    #***********************************************************************************************************
    downloads_all = []
    downloads_data = [[u['URL'] for u in r['umm']['RelatedUrls'] if
                       u['Type'] == "USE SERVICE API" and ('Subtype' in u and u['Subtype'] == "OPENDAP DATA")] for r in
                      results['items']]
    ...
    for f in downloads:
        try:
            ...
            output_path = join(data_path, basename(f))+'_subset.nc'
            ...
            urlretrieve(f+'.dap.nc?dap4.ce='+strOpendap[0:-1], output_path)
            pa.process_file(process_cmd, output_path, args)
